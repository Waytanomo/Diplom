...
namespace TicketMonitor.Infrastructure.Services
{
    public class TicketService : ITicketService
    {
        ...
        {
            var ticket = new Ticket { Title = dto.Title, Description = dto.Description, Priority = dto.Priority, CreatedById = userId };
        ...
        public async Task<bool> ChangeStatusAsync(int id, ChangeStatusDto dto, string userId)
        {
            ...
            ticket.ClosedAt = (dto.NewStatus == TicketStatus.Closed || dto.NewStatus == TicketStatus.Resolved) ? DateTime.UtcNow : null;

            _context.StatusLogs.Add(new StatusLog { TicketId = id, OldStatus = oldStatus, NewStatus = dto.NewStatus, ChangedById = userId });
            await _context.SaveChangesAsync();
            return true;
        }
        ...
        public async Task<bool> AssignAsync(int id, AssignTicketDto dto, string userId)
        {
            ...
            // ✅ Явно указан тип <ApplicationUser>, чтобы компилятор точно выбрал EF Core-версию
        ...
        {
            // ✅ Явно указан тип <Ticket>
            if (!await _context.Tickets.AnyAsync<Ticket>(t => t.Id == id)) throw new KeyNotFoundException("Ticket not found");

            var comment = new Comment { TicketId = id, Text = dto.Text, AuthorId = userId };
            _context.Comments.Add(comment);
            await _context.SaveChangesAsync();

            return new CommentDto(comment.Id, comment.Text, comment.Author.UserName ?? "Unknown", comment.CreatedAt);
        }
        ...
        public async Task<TicketStatsDto> GetStatsAsync()
        {
            ...
            var byStatus = await _context.Tickets.GroupBy(t => t.Status)
                .Select(g => new { Status = g.Key.ToString(), Count = g.Count() })
                .ToDictionaryAsync(x => x.Status, x => x.Count);

            var byPriority = await _context.Tickets.GroupBy(t => t.Priority)
                .Select(g => new { Priority = g.Key.ToString(), Count = g.Count() })
                .ToDictionaryAsync(x => x.Priority, x => x.Count);

            var byAssignee = await _context.Tickets.Where(t => t.AssignedTo != null)
                .GroupBy(t => t.AssignedTo!.UserName)
                .Select(g => new { Assignee = g.Key!, Count = g.Count() })
                .ToDictionaryAsync(x => x.Assignee, x => x.Count);

            var closed = await _context.Tickets.Where(t => t.ClosedAt.HasValue).ToListAsync();
            var avgHours = closed.Any() ? closed.Average(t => (t.ClosedAt!.Value - t.CreatedAt).TotalHours) : 0.0;
        ...
        private static TicketDto MapToDto(Ticket t) => new(
            t.Id, t.Title, t.Description, t.Status, t.Priority, t.CreatedAt, t.ClosedAt,
            t.AssignedTo?.UserName, t.CreatedBy.UserName ?? "Unknown");
    }
}